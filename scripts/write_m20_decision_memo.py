"""Operator helper for research-only M20 decision memo."""
...
def main() -> None:
    """Write M20 decision memo artifacts."""
    parser = argparse.ArgumentParser(description="Write M20 research decision memo")
    parser.add_argument("--base-run-dir", required=True)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    result = write_m20_decision_memo(base_run_dir=Path(args.base_run_dir))
    if args.json:
