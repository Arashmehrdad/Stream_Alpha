"""Operator helper for M20 rank-gate plus momentum combo diagnostics."""
...
def main() -> None:
    """Run M20 gate + momentum combo diagnostics."""
    parser = argparse.ArgumentParser(description="Analyze M20 gate momentum combos")
    parser.add_argument("--base-run-dir", required=True)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    result = analyze_m20_gate_momentum_combo(base_run_dir=Path(args.base_run_dir))
    if args.json:
