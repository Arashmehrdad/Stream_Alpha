"""Research-only M20 volatility-expansion setup diagnostics."""
...
OUTPUT_DIR_NAME = "volatility_expansion_research"
...
REQUIRED_FEATURES = ("realized_vol_12", "high_price", "low_price", "close_price", "volume")


def analyze_m20_volatility_expansion(*, base_run_dir: Path) -> dict[str, Any]:
    """Analyze volatility-expansion setup diagnostics across available M20 windows."""
    ...
    for run_label, run_dir in runs:
        ...
        for setup_name, predicate in _setup_specs(rows):
            setup_metrics.append(_metric_row(run_label, setup_name, rows, predicate))
    ...
    Path(output_files["report_md"]).write_text(_markdown(report, setup_metrics), encoding="utf-8")
...
def _joined_rows(run_dir: Path) -> list[dict[str, Any]]:
    features = {
        _key(row): row
        for row in _read_csv(run_dir / "training_frame" / "m20_training_frame_features.csv")
    }
    labels = {
        _key(row): row
...
def _setup_specs(
    ...
    _vol_mid, vol_high = _tertiles([_float(row.get("realized_vol_12")) for row in rows])
    _volume_mid, volume_high = _tertiles([_float(row.get("volume")) for row in rows])
    _range_mid, range_high = _tertiles([_float(row.get("range_pct")) for row in rows])
    abs_return_high = _quantile([abs(_float(row.get("log_return_1"))) for row in rows], 2 / 3)
    macd_abs_high = _quantile([abs(_float(row.get("macd_line_12_26"))) for row in rows], 2 / 3)
    specs: list[tuple[str, Callable[[Mapping[str, Any]], bool]]] = [
        ("realized_vol_high", lambda row: _float(row.get("realized_vol_12")) >= vol_high),
        ("range_high", lambda row: _float(row.get("range_pct")) >= range_high),
        ("volume_high", lambda row: _float(row.get("volume")) >= volume_high),
        (
            "vol_plus_range_high",
            lambda row: (
                _float(row.get("realized_vol_12")) >= vol_high
                and _float(row.get("range_pct")) >= range_high
            ),
        ),
        (
            ...
            lambda row: (
                _float(row.get("realized_vol_12")) >= vol_high
                and _float(row.get("volume")) >= volume_high
            ),
        ),
        (
            "range_plus_volume_high",
            lambda row: (
                _float(row.get("range_pct")) >= range_high
                and _float(row.get("volume")) >= volume_high
            ),
        ),
    ]
    if _has_values(rows, "log_return_1"):
        specs.append(
            (
                "abs_log_return_high",
                lambda row: abs(_float(row.get("log_return_1"))) >= abs_return_high,
            )
        )
    if _has_values(rows, "lag_log_return_1") and _has_values(rows, "log_return_1"):
        specs.extend(
            [
                (
                    "shock_continuation",
                    lambda row: (
                        abs(_float(row.get("log_return_1"))) >= abs_return_high
                        and _float(row.get("log_return_1")) * _float(row.get("lag_log_return_1"))
                        > 0.0
                    ),
                ),
                (
    ...
    if _has_values(rows, "macd_line_12_26"):
        specs.append(
            (
                "macd_expansion",
                lambda row: abs(_float(row.get("macd_line_12_26"))) >= macd_abs_high,
            )
        )
    return specs
...
def _metric_row(
    ...
    return {
        ...
        "net_proxy_sum": sum(_float(row.get("net_proxy")) for row in setup_rows),
        "net_proxy_mean": (
            sum(_float(row.get("net_proxy")) for row in setup_rows) / len(setup_rows)
...
def _overlap_row(
    ...
    setup_rows = [row for row in rows if predicate(row)]
    overlap = [row for row in setup_rows if _rank_gate_key(run_label, row) in rank_gate_keys]
    return {
        "run_label": run_label,
        "setup_name": setup_name,
        "setup_rows": len(setup_rows),
...
def _recommendation(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    recommendation = (
        ...
        if stable else "NO_STABLE_VOLATILITY_EXPANSION_SETUP_FOUND"
    )
    return {
...
def _rank_gate_keys(base_dir: Path) -> set[tuple[str, str, str]]:
    ...
    return {
        _rank_gate_key(str(row.get("run_label", "")), row)
...
def _markdown(report: Mapping[str, Any], metrics: Sequence[Mapping[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# M20 Volatility Expansion Research Diagnostic",
            "",
            f"- Recommendation: `{report['recommendation']}`",
            f"- Best observed setup: `{best.get('setup_name', '')}`",
            f"- Honesty flags: `{', '.join(HONESTY_FLAGS)}`",
            "",
            "This is setup diagnostics only, not a backtest, PnL, or trading logic.",
            "",
        ]
    )
...
def _rank_gate_key(run_label: str, row: Mapping[str, Any]) -> tuple[str, str, str]:
    return (run_label, str(row.get("symbol", "")), str(row.get("interval_begin", "")))
...
def _net_proxy(row: Mapping[str, Any]) -> float:
    for column in ("net_proxy", "net_return_proxy", "net_forward_return", "net_return"):
        if column in row:
            return _float(row.get(column))
    if "future_return" in row and "cost_per_trade" in row:
        return _float(row.get("future_return")) - _float(row.get("cost_per_trade"))
    return 0.0
...
def _has_values(rows: Sequence[Mapping[str, Any]], column: str) -> bool:
    return any(column in row and math.isfinite(_float(row.get(column))) for row in rows)
...
def read_volatility_expansion_csv(path: Path) -> list[dict[str, str]]:
    """Read volatility-expansion diagnostic CSV for tests."""
    with path.open("r", encoding="utf-8", newline="") as handle:
        return [dict(row) for row in csv.DictReader(handle)]
