"""Research-only M20 momentum-breakout setup diagnostics."""
...
OUTPUT_DIR_NAME = "momentum_breakout_research"
...
def analyze_m20_momentum_breakout(*, base_run_dir: Path) -> dict[str, Any]:
...
def _setup_specs(
    ...
    volume_mid, volume_high = _tertiles([_float(row.get("volume")) for row in rows])
    range_mid, range_high = _tertiles([_float(row.get("range_pct")) for row in rows])
    return [
        ("macd_positive", lambda row: _float(row.get("macd_line_12_26")) > 0.0),
        ("return_momentum_positive", lambda row: _momentum(row) > 0.0),
        ("return_momentum_flat", lambda row: abs(_momentum(row)) <= 0.0005),
        ...
        ("breakout_proxy", _breakout_proxy),
    ]
...
def _recommendation(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    recommendation = (
        ...
        if stable else "NO_STABLE_MOMENTUM_BREAKOUT_SETUP_FOUND"
    )
    return {
...
def _markdown(report: Mapping[str, Any], metrics: Sequence[Mapping[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# M20 Momentum Breakout Research Diagnostic",
            "",
...
def _breakout_proxy(row: Mapping[str, Any]) -> bool:
    return (
        _float(row.get("close_zscore_12")) > 1.0
        or (
            _float(row.get("close_price")) > _float(row.get("vwap"))
            and _momentum(row) > 0.0
        )
    )
...
def read_momentum_csv(path: Path) -> list[dict[str, str]]:
