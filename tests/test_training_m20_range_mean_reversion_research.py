"""Focused tests for M20 range mean-reversion research diagnostics."""
...
def test_range_mean_reversion_writes_outputs(tmp_path: Path) -> None:
    base = tmp_path / "run"
    _write_run(base)

    result = analyze_m20_range_mean_reversion(base_run_dir=base)

    assert "RESEARCH_ONLY" in result["honesty_flags"]
    assert result["setup_count"] >= 7
    assert Path(result["output_files"]["setup_metrics_csv"]).exists()
    assert Path(result["output_files"]["rank_gate_overlap_csv"]).exists()


def test_range_mean_reversion_blocks_missing_required_features(tmp_path: Path) -> None:
    base = tmp_path / "run"
    _write_json(
